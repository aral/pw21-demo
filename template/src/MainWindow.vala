namespace PW21demo {
    public class MainWindow : Hdy.ApplicationWindow {
        public weak PW21demo.Application app { get; construct; }

        // Widgets
        public PW21demo.Widgets.HeaderBar toolbar;
        ...
        public MainWindow (PW21demo.Application application) {
            Object (
                ...
                icon_name: "com.github.aral.pw21_demo"
            );
        }
        ...
        private void create_layout () {
            ...
            toolbar = new PW21demo.Widgets.HeaderBar ();
        ...
        private void restore_window_state () {
            ...
            PW21demo.saved_state.get ("window-size", "(ii)", out rect.width, out rect.height);

            default_width = rect.width;
            default_height = rect.height;

            var window_state = PW21demo.saved_state.get_enum ("window-state");
            switch (window_state) {
                ...
                case WindowState.FULLSCREEN:
                    ...
                    break;
                default:
                    PW21demo.saved_state.get ("window-position", "(ii)", out rect.x, out rect.y);
        ...
        private void preserve_window_state () {
            ...
            if (Gdk.WindowState.MAXIMIZED in state) {
                PW21demo.saved_state.set_enum ("window-state", WindowState.MAXIMIZED);
            } else if (Gdk.WindowState.FULLSCREEN in state) {
                PW21demo.saved_state.set_enum ("window-state", WindowState.FULLSCREEN);
            } else {
                PW21demo.saved_state.set_enum ("window-state", WindowState.NORMAL);
                // Save window size
                int width, height;
                get_size (out width, out height);
                PW21demo.saved_state.set ("window-size", "(ii)", width, height);
            }

            int x, y;
            get_position (out x, out y);
            PW21demo.saved_state.set ("window-position", "(ii)", x, y);
        }
